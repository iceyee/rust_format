...
impl HtmlFormatter {
    ...
    unsafe fn split_text(text: &[u8]) {
        ...
        let mut x: usize = 0;
        let mut is_style: bool = false;
        let mut is_script: bool = false;
        while x < text.len() {
            match split_state {
                SplitState::Neutral => {
                    if 0 < word.len() {
                        let w: String = String::from_utf8(word.clone()).unwrap();
                        if w == "<style" {
                            is_style = true;
                        } else if w == "<script" {
                            is_script = true;
                ...
                SplitState::Content => {
                    ...
                    if is_style {
                        split_state = SplitState::Style;
                        x -= 1;
                    } else if is_script {
                        split_state = SplitState::Script;
                        x -= 1;
                    } else {
    ...
    unsafe fn rebuild_text() {
        ...
        HtmlFormatter::buffer_roll_new(WORDS[0 + 1].clone(), TYPES[0 + 1].clone());
        HtmlFormatter::buffer_roll_new(WORDS[0 + 2].clone(), TYPES[0 + 2].clone());
        HtmlFormatter::buffer_roll_new(WORDS[0 + 3].clone(), TYPES[0 + 3].clone());
        while x < WORDS.len() {
            if x + 4 < WORDS.len() {
                HtmlFormatter::buffer_roll_new(WORDS[x + 4].clone(), TYPES[x + 4].clone());
            } else {
                HtmlFormatter::buffer_roll_new(String::new(), WordType::Space);
            }
            ...
            {
                TEXT.push('\n');
                HtmlFormatter::append_indent(INDENT);
            } else if TYPES[x] == WordType::Style || TYPES[x] == WordType::Script {
                TEXT.push('\n');
            }
            ...
            HtmlFormatter::buffer_roll_old(WORDS[x].clone(), TYPES[x].clone());
            x += 1;
        }
